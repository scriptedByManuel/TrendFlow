...
const WishList = () => {
    ...
    useEffect(() => {
        if (!user?.id) return // don't subscribe until user exists

        const channel = subscribeToWishListChanges(user.id, async () => {
            const updated = await getWishList(user.id)
            setWishListItems(updated)
        })

        return () => unsubscribeFromWishListChanges(channel)
    }, [user?.id])
